...
class Compass(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), amp_fac=2, eps=1e-8, weight_decay=0):
        defaults = dict(lr=lr, betas=betas, amp_fac=amp_fac, eps=eps, weight_decay=weight_decay)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                denom = ema_squared.sqrt().add_(group['eps'])
                ...
                # Weight decay on grad just like RMSProp
                # TODO: not sure if this thing is correct! need to test first!
                if weight_decay != 0:
                    # grad = 
